...
class UserController extends Controller
{
    ...
    {
        // 1️⃣ Validez les données de la requête
        $validatedData = $request->validate([
            'speudo' => 'required|string|max:255',
        ...
        try {
            // 2️⃣ Recherchez l'utilisateur correspondant au speudo et motDePasse
            $user = User::where('speudo', $validatedData['speudo'])
                        ->where('motDePasse', $validatedData['motDePasse'])
                        ->first();
        
            // 3️⃣ Vérifiez si l'utilisateur existe
            if (is_null($user)) {
                return response()->json(['error' => 'Identifiants incorrects'], 401);
            }
        
            // 4️⃣ Retournez l'utilisateur connecté
